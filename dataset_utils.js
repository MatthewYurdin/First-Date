...
const union = (x, y) => distinct(glue(x, y));

//Return unique elements found in both x and y
const intersection = (x, y) => union(x, y).filter(f => ((includes(x, f)) && (includes(y, f))));

//Return array x minus array y
const difference = ( x, y ) => {
  const inter = intersection( x, y );
  return union(x, y).filter(f => includes(inter, f) === false);
};